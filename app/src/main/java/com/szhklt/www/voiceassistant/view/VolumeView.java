...
//    private static final int SMALL_WAVE_COLOR = 0xFF77DAFD;//小波颜色
    private static final int SMALL_WAVE_COLOR = Color.rgb(238,233,191);
    
//    private static final int BIG_WAVE_COLOR = 0xFF4DA6F3;//大波颜色
    private static final int BIG_WAVE_COLOR = Color.rgb(248,248,255);
    
    private static final float SMALL_WAVE_SPEED = 1.5f;//小波速度

    private static final float BIG_WAVE_SPEED = 2.0f;//大波速度
    ...
    private static final float SMALL_AMPLITUDE_SIZE_SCALE = 10f;

    private static final float BIG_AMPLITUDE_SIZE_SCALE = 8f;

    private float mVerticalSpeed; // (中间位置)垂直扩展时的移动速度，每秒移动像素数

    private float mVerticalRestoreSpeed; // (中间位置)垂直恢复时的移动速度，每秒移动像素数

    private static final float MAX_VALUE = 300;// 最大振幅对应的值。对应0到100分贝
    ...
    private void drawWave(Canvas canvas, Path path, Paint paint, int period,
        ...
        drawSine(canvas, mPath, mPaint, BIG_WAVE_PERIOD, mDrawWidth,
                (mVolumeAmplitude * mMaxBigWaveAmplitude)+(MainService.volume_value/5), mPhase
        ...
        drawSine(canvas, mPath, mPaint, SMALL_WAVE_PERIOD, mDrawWidth,
                (mVolumeAmplitude * mMaxSmallWaveAmplitude)+(MainService.volume_value/5), mPhase
