 ...
 * 闹钟列表页面的时钟
 */
public class AlarmListClock extends View {
	...
	private float mColorLineLenght = 13;
	private int mColorLineNum = 150;
	private float mPointRadius = 9;//红点半径
	private float outPadding = 15;//外圈大圆与边界距离（突出部分高度）
	...
	public AlarmListClock(Context context) {
		this(context, null);
	}

	public AlarmListClock(Context context,  AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public AlarmListClock(Context context,  AttributeSet attrs, int defStyleAttr) {
...
//		mPaint.setTypeface(typeface);
		...
		float baseLine =mCenterY-textBound.top-textBound.height()/2;//使数字时间显示在圆圈中间
